...
if (!isset($data['requirement']) || $data['requirement']) {
    ...
    echo (sprintf(
        '<li class="nav-item %s"><a href="%s" %s>%s%s</a>%s</li>',
        (empty($data['children']) ? '' : 'dropdown'),
        (empty($data['url']) ? '#' : h($data['url'])),
        (empty($data['children']) ? 'class="nav-link"' : 'class="nav-link dropdown-toggle" data-bs-toggle="dropdown"'),
