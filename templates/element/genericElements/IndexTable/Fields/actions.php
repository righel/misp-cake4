...
foreach ($actions as $action) {
    ...
    if (isset($action['postLink'])) {
        echo $this->Form->postLink(
            ...
            $url,
            array(
                'class' => 'btn btn-sm btn-outline-dark table-link-action ' . $this->FontAwesome->getClass($action['icon']) . (empty($action['class']) ? '' : h($action['class'])),
        ...
        echo sprintf(
            ...
            empty($action['title']) ? '' : h($action['title']),
            empty($action['dbclickAction']) ? '' : 'dblclickActionElement',
