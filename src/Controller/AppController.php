...
class AppController extends Controller
{
    ...
    public function initialize(): void
    {
        ...
        // TODO: [cakephp 2.x -> 4.x migration] (load components)
        $components = array(
            'Session',
            'Auth' => array(
    ...
    public function beforeFilter(EventInterface $event)
    {
        ...
        if (Configure::read('Security.check_sec_fetch_site_header')) {
            $secFetchSite = $this->request->getHeader('Sec-Fetch-Site');
            if ($secFetchSite !== false && $secFetchSite[0] !== 'same-origin' && ($this->request->is('post') || $this->request->is('put') || $this->request->is('ajax'))) {
        ...
        if (!$this->_isRest()) {
            ...
            $this->response->getHeader('X-XSS-Protection', '1; mode=block');
        }

        // TODO: [cakephp 2.x -> 4.x migration] (named params)
        ...
        // TODO: [cakephp 2.x -> 4.x migration] using Authentication component
        // For fresh installation (salt empty) generate a new salt
        // if (!Configure::read('Security.salt')) {
        //     $this->loadModel('Servers');
        //     $this->Servers->serverSettingsSaveValue('Security.salt', $this->User->generateRandomPassword(32));
        // }

        // // Check if the instance has a UUID, if not assign one.
        // if (!Configure::read('MISP.uuid')) {
        //     $this->loadModel('Servers');
        //     $this->Servers->serverSettingsSaveValue('MISP.uuid', CakeText::uuid());
        // }
        ...
        Configure::write('CurrentController', $this->request->getParam('controller'));
        Configure::write('CurrentAction', $this->request->getParam('action'));
        ...
        if ($this->_isRest()) {
            ...
            // TODO: [cakephp 2.x -> 4.x migration] using BodyParserMiddleware
            // read: https://book.cakephp.org/3/en/controllers/middleware.html#body-parser-middleware
            // read: https://book.cakephp.org/3/en/controllers/components/request-handling.html#automatically-decoding-request-data
        ...
        // TODO: [cakephp 2.x -> 4.x migration] using AuthenticationComponent
        // if (
        //     !$userLoggedIn &&
        //     (
    ...
    private function __contentSecurityPolicy()
    {
        ...
        $this->response->getHeader($headerName, implode('; ', $header));
    }
    ...
    private function __rateLimitCheck()
    {
        ...
        if ($rateLimitCheck !== true) {
            $this->response->withHeader('X-Rate-Limit-Limit', $info['limit']);
            $this->response->withHeader('X-Rate-Limit-Remaining', $info['remaining']);
            $this->response->withHeader('X-Rate-Limit-Reset', $info['reset']);
            $this->response->withBody($rateLimitCheck);
    ...
    protected function _setupDatabaseConnection()
    {
        ...
        if (!isset($dataSourceConfig['encoding'])) {
            ...
            $db->config(['encoding' => 'utf8']);
            // ConnectionManager::drop('default');
            // ConnectionManager::setConfig('default', $db->config());
        }
        $dataSource = $dataSourceConfig['driver'];
        if (!in_array($dataSource, [
            'Cake\Database\Driver\Mysql',
            'Cake\Database\Driver\Postgres',
            'Cake\Database\Driver\MysqlObserver'
        ])) {
    ...
    {
        if (isset($this->userRole) && ($this->userRole['perm_site_admin'] || $this->userRole['perm_admin'])) {
            return true;
        }
        return false;
    }
    ...
    {
        return isset($this->userRole) && $this->userRole['perm_site_admin'];
    }
    ...
    /**
     * Set the ACL and user layout variables.
     *
     * @return void
     */
