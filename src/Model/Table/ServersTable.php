 ...
 */
class ServersTable extends AppModel
{
    ...
    public function initialize(array $config): void
    {
        ...
        $this->belongsTo('RemoteOrganisations', [
            'foreignKey' => 'remote_organisation_id',
            'joinType' => 'INNER',
        ]);
        $this->hasMany('Users', [
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator
            ->scalar('authkey')
            ->maxLength('authkey', 40)
            ->requirePresence('authkey', 'create')
            ->notEmptyString('authkey');

        $validator
            ->boolean('push')
            ->requirePresence('push', 'create')
            ->notEmptyString('push');

        $validator
            ->boolean('pull')
            ->requirePresence('pull', 'create')
            ->notEmptyString('pull');

        $validator
            ->boolean('push_sightings')
            ->notEmptyString('push_sightings');

        $validator
            ->boolean('push_galaxy_clusters')
            ->notEmptyString('push_galaxy_clusters');

        $validator
            ->boolean('pull_galaxy_clusters')
            ->notEmptyString('pull_galaxy_clusters');
        ...
        $validator
            ->scalar('organization')
            ->maxLength('organization', 10)
            ->allowEmptyString('organization');

        $validator
            ->boolean('publish_without_email')
            ->notEmptyString('publish_without_email');

        $validator
            ->boolean('unpublish_event')
            ->notEmptyString('unpublish_event');

        $validator
            ->boolean('self_signed')
            ->requirePresence('self_signed', 'create')
            ->notEmptyString('self_signed');

        $validator
            ->scalar('pull_rules')
            ->requirePresence('pull_rules', 'create')
            ->notEmptyString('pull_rules');

        $validator
            ->scalar('push_rules')
            ->requirePresence('push_rules', 'create')
            ->notEmptyString('push_rules');
        ...
        $validator
            ...
            ->notEmptyString('caching_enabled');
    ...
    public function buildRules(RulesChecker $rules): RulesChecker
    {
        ...
        $rules->add($rules->existsIn(['remote_organisation_id'], 'RemoteOrganisations'), ['errorField' => 'remote_organisation_id']);
    ...
    public function getCurrentServerSettings()
    {
        ...
        // TODO: [cakephp 2.x -> 4.x migration] and remove this
    ...
    public function serverSettingsSaveValue($setting, $value)
    {
        ...
        $current = file_get_contents(APP . '../config' . DS . 'app_local.php');
        $current = trim($current);
        if (strlen($current) < 20) {
            // TODO: [cakephp 2.x -> 4.x migration]
            // $this->Log = ClassRegistry::init('Logs');
            // $this->Log->create();
            // $this->Log->save(array(
            //     'org' => 'SYSTEM',
            //     'model' => 'Server',
        ...
        if ($safeConfigChanges) {
            ...
            copy(APP . '../config' . DS . 'app_local.php', APP . '../config' . DS . 'app_local.php.bk');
        }
        ...
        if ($safeConfigChanges) {
            $previous_file_perm = substr(sprintf('%o', fileperms(APP . '../config' . DS . 'app_local.php')), -4);
            ...
            rename(APP . '../config' . DS . $randomFilename, APP . '../config' . DS . 'app_local.php');
            if (function_exists('opcache_reset')) {
                opcache_reset();
            }
            chmod(APP . '../config' . DS . 'app_local.php', octdec($previous_file_perm));
            $config_saved = file_get_contents(APP . '../config' . DS . 'app_local.php');
            // if the saved config file is empty, restore the backup.
            if (strlen($config_saved) < 20) {
                copy(APP . '../config' . DS . 'app_local.php.bk', APP . '../config' . DS . 'app_local.php');
                $this->Log = ClassRegistry::init('Log');
                $this->Log->create();
                $this->Log->save(array(
                    'org' => 'SYSTEM',
            ...
            file_put_contents(APP . '../config' . DS . 'app_local.php', $settingsString);
            if (function_exists('opcache_reset')) {
                opcache_reset();
            }
        }
