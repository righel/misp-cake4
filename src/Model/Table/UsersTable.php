...
class UsersTable extends AppModel
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator
            ->boolean('autoalert')
            ->notEmptyString('autoalert');

        $validator
            ->scalar('authkey')
            ->maxLength('authkey', 40)
            ->allowEmptyString('authkey');
        ...
        $validator
            ->scalar('gpgkey')
            ->maxLength('gpgkey', 4294967295)
            ->allowEmptyString('gpgkey');

        $validator
            ->scalar('certif_public')
            ->maxLength('certif_public', 4294967295)
            ->allowEmptyString('certif_public');

        $validator
            ->integer('nids_sid')
            ->notEmptyString('nids_sid');

        $validator
            ->boolean('termsaccepted')
            ->notEmptyString('termsaccepted');

        $validator
            ->nonNegativeInteger('newsread')
            ->allowEmptyString('newsread');

        $validator
            ->notEmptyString('change_pw');

        $validator
            ->boolean('contactalert')
            ->notEmptyString('contactalert');
        ...
        $validator
            ->integer('current_login')
            ->allowEmptyString('current_login');
        ...
        $validator
            ...
            ->notEmptyString('force_logout');

        $validator
            ->allowEmptyString('date_created');

        $validator
            ->allowEmptyString('date_modified');
            ...
            if (!empty($this->data)) {
                ...
                if (isset($user['User']['id'])) {
                    $user = $this->find('first', array(
                        'recursive' => -1,
                        'conditions' => array('Users.id' => $user['User']['id']),
    ...
    {
        $orgs = $this->Organisations->find('list', array(
            'recursive' => -1,
            'fields' => array('name'),
        ));
        return $orgs;
    ...
    public function getUsersWithAccess($owners = array(), $distribution, $sharing_group_id = 0, $userConditions = array())
    {
        ...
        $users = $this->find('all', array(
            ...
            'contain' => ['Roles' => ['fields' => ['perm_site_admin', 'perm_audit']], 'Organisations' => ['fields' => ['id', 'name']]],
        ));
        foreach ($users as $k => $user) {
            $user = $user['User'];
            unset($users[$k]['User']);
    ...
    public function describeAuthFields()
    {
        ...
        foreach ($this->associations() as $relatedModel) {
            $fields[] = $relatedModel->getName() . '.*';
        }
        return $fields;
    }
    ...
    public function findAdminsResponsibleForUser($user)
    {
        $admin = $this->find('first', array(
            ...
            'contain' => array(
                'Roles' => array('fields' => array('perm_admin'))
            ),
            'fields' => array('Users.id', 'Users.email', 'Users.org_id')
        ));
        if (count($admin) == 0) {
            $admin = $this->find('first', array(
                'recursive' => -1,
                'conditions' => array(
                    'Roles.perm_site_admin' => 1,
                    'Users.disabled' => 0,
                ),
                'contain' => array(
                    'Roles' => array('fields' => array('perm_site_admin'))
                ),
                'fields' => array('Users.id', 'Users.email', 'Users.org_id')
            ));
        }
    ...
    public function registerUser($added_by, $registration, $org_id, $role_id)
    {
        ...
        $this->create();
        $this->Log = TableRegistry::getTableLocator()->get('Logs');
        $result = $this->save(array('User' => $user));
        $currentOrg = $this->Organisations->find('first', array(
            'recursive' => -1,
        ...
        if (!empty($currentOrg) && empty($currentOrg['Organisation']['local'])) {
            ...
            $this->Organisations->save($currentOrg);
        }
        if (empty($result)) {
            $error = array();
